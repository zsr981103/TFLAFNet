...
#syn
path = 'data/sgy_data/2007BP_part4_11shot.sgy'
origin_o, nSample_o,extent_time_o = get_info_seg(path)
train_data, _, extent_time_t = get_info_seg('data/2007BP_synthetic_train.sgy')
origin = train_data[:, -800:]   # 最后一炮 800 道
extent_time = extent_time_o
...
noise_data_original = noisy_shot
...
# data_sea, seismic_time_sea, time_length_sea = get_info_seg(data_sea_path)
# origin = data_sea
# extent_time = time_length_sea
#
# noise_data = get_mat('data/field_data/Sea_0_1_shot.mat')
# noise_data_original = noise_data
scale = 1

print(calculate_snr(origin,noise_data))
print(calculate_rmse(origin,noise_data))
...
model = ResEncLapDecNet(in_channels=C).to(device)


# model = DnCNN(in_channels=C).to(device)
# model = APCNet(in_channels=C).to(device)
...
# model = UNet(in_channels=C).to(device)
# model = UNet_with_Res(in_channels=C).to(device)
# model = UNet_Skip_with_Lap(in_channels=C).to(device)
# model = UNet_Up_with_Lap(in_channels=C).to(device)
# model = UNet_with_CA(in_channels=C).to(device)
# model = UNet_Skip_Up_with_Lap(in_channels=C).to(device)



weights_path = "data/record_result/part4/APCNet/model.pth"

# model.load_state_dict(torch.load(weights_path, map_location=device))

state_dict = torch.load(weights_path, map_location=device, weights_only=True)
# cached_kernel 是运行时 buffer，首次 forward 会自动重建，加载时跳过
state_dict = {k: v for k, v in state_dict.items() if "cached_kernel" not in k}
