...
from UNet_DE_Res import *
from UNet_DRes import *

from LUNet_ELU import *
from LUNet import *
from ATUNet import *
from APCNet import *
# 定义是否使用GPU
...
BATCH_SIZE_s = 50  # 批处理尺寸(batch_size)
BATCH_SIZE_v = 50
LR = 0.0001  # 0.000012
rate = 0.5  # 学习率衰变
iteration = 40  # 每10次衰减
...
data = data[:, -4000:-800]
time_length = [0, data.shape[1], time_length[2], 0]

data_sea_path = 'data/field_data/Sea_0_1_shot.sgy'
data_sea, seismic_time_sea, time_length_sea = get_info_seg(data_sea_path)
# clean_data = data
###VMD噪声###
np.random.seed(417)
...
print(data.shape)
print(noise.shape)
# data: (n_time, n_traces)，如 (1151, 4000)
# noise: (n_time_noise, n_traces_noise)，如 (1500, 119)；沿 axis=0 为时间
...
if vmd == True:
    # -----------------------------
    # 不同炮集对应不同噪声系数（按地震道序号分炮）
    # data: (n_time, 4000)，5 炮 × 800 道
    # -----------------------------
    shot_size = 800
    alphas = [0.9, 0.95, 1.05, 1.1]
    n_traces = data.shape[1]
    n_shots = n_traces // shot_size
    if n_traces % shot_size != 0:
        raise ValueError(
            f"震道数 {n_traces} 不能被 shot_size={shot_size} 整除，无法按炮分块"
        )
    if len(alphas) != n_shots:
        raise ValueError(
            f"alphas 长度 {len(alphas)} 与炮集数 {n_shots} 不一致"
        )

    noisy_data = np.array(data, dtype=np.float64, copy=True)
    for shot_idx in range(n_shots):
        lo = shot_idx * shot_size
        hi = lo + shot_size
        syn_shot = data[:, lo:hi]
        noisy_shot = add_field_noise(
            noise=noise,
            field_data=data_sea,
            syn_data=syn_shot,
            noise_scale=alphas[shot_idx],
            map_mode="proportional",
            print_mapping=False,
            print_mismatch=False,
        )
        noisy_data[:, lo:hi] = noisy_shot
        # plot_seismic_npy(noisy_shot, time_length, show=True)


        print(
            f"炮集 {shot_idx + 1}/{n_shots}: 道 [{lo}:{hi}), "
            f"noise_scale={alphas[shot_idx]}"
        )

    # print("使用VMD噪声（按炮集不同强度）")
    # data_1 = noisy_data[:, -800:]
    # data_2 = original_clean_data[:, -800:]
    # time_length = [0, data.shape[1], time_length[2], 0]
    #
    # plot_seismic_npy(data_1, time_length, show=True)
    # plot_seismic_npy(data_2, time_length, show=True)

    original_noise_data = noisy_data
    print(calculate_snr(original_clean_data, original_noise_data))
    clean_data = np.expand_dims(data, axis=-1)  # 在最后加一个维度 (采样点, 震道, 1)
    ...
    noise_data, clean_data = predict_data_extract_paired_patches(clean_data=original_clean_data, noise_data=original_noise_data,
                                                    patch_length=Data_Size, stride=128)
else:
    print("使用原始未分解数据")
    sigma = 700
...
from Kalman_ResNet import *
# net = KResNet18(in_channels=input_channels).to(device)

from model import *
net = ResEncLapDecNet(in_channels=input_channels).to(device)
# net = ResEncLapDecNet_ELU(in_channels=input_channels).to(device)
# net = UNet_DE_Res(in_channels=input_channels).to(device)
# net = UNet_DRes(in_channels=input_channels).to(device)
# net = ATUNet(in_channels=input_channels).to(device)
# net = LUNet(in_channels=input_channels).to(device)
# net = LUNet_ELU(in_channels=input_channels).to(device)


from UNet_with_Res import *
from UNet_Skip_with_Lap import *
from UNet_Up_with_Lap import *
from UNet_Skip_Up_with_Lap import *
from UNet_with_CA import *

# net = UNet(in_channels=input_channels).to(device)
# net = UNet_with_Res(in_channels=input_channels).to(device)
# net = UNet_Skip_with_Lap(in_channels=input_channels).to(device)
# net = UNet_Up_with_Lap(in_channels=input_channels).to(device)
# net = UNet_with_CA(in_channels=input_channels).to(device)
# net = UNet_Skip_Up_with_Lap(in_channels=input_channels).to(device)


# net = ResNet18(in_channels=input_channels).to(device)
# net = APCNet(in_channels=input_channels).to(device)
# net = DnCNN(in_channels=input_channels).to(device)
# net = FDConv1D(in_channels=1, out_channels=1, kernel_size=3, padding=1, bias=True, use_fdconv_if_c_gt=0).to(device)
